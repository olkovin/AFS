...
function Create-FolderStructure {
    ...
    Write-Log -Message "Creating folder structure. Language: $Lang, NamingPattern: $NamingPattern" -LogPath $logPath
    
    $year = $Date.Year.ToString()
    $month = if ($NamingPattern -match "(?i)month") {
        if ($Lang -eq "UA") {
            @("Січень", "Лютий", "Березень", "Квітень", "Травень", "Червень", 
              "Липень", "Серпень", "Вересень", "Жовтень", "Листопад", "Грудень")[$Date.Month - 1]
        } else {
    ...
    $yearPath = Join-Path -Path $BaseFolder -ChildPath $year
    $monthPath = Join-Path -Path $yearPath -ChildPath $month
    $dayPath = Join-Path -Path $monthPath -ChildPath $day
    
    Write-Log -Message "Created folder path: $dayPath" -LogPath $logPath
...
function Sort-Files {
    ...
    Write-Log -Message "Sort-Files started. Language: $lang" -LogPath $logPath
    Write-Log -Message "Current directory: $(Get-Location)" -LogPath $logPath
    Write-Log -Message "PSScriptRoot: $PSScriptRoot" -LogPath $logPath
    
    $srcPath = if ($Config.src_path) { 
        $fullSrcPath = [System.IO.Path]::GetFullPath($Config.src_path)
        Write-Log -Message "Full source path: $fullSrcPath" -LogPath $logPath
        $fullSrcPath
    } else { 
        $PSScriptRoot 
    }
    ...
    $dstPath = if ($Config.dst_path) { 
        $fullDstPath = [System.IO.Path]::GetFullPath($Config.dst_path)
        Write-Log -Message "Full destination path: $fullDstPath" -LogPath $logPath
        $fullDstPath
    } else { 
        $srcPath 
    }
    
    Write-Log -Message "Final Source path: $srcPath" -LogPath $logPath
    Write-Log -Message "Final Destination path: $dstPath" -LogPath $logPath
    
    if (-not (Test-Path $srcPath)) {
        Write-Log -Message "Source path does not exist: $srcPath" -LogPath $logPath -Level "ERROR"
        return
    }
    
    # Прямий пошук файлів
    $searchPattern = if ($Config.search_filter_type -eq "extension") {
        "*.$($Config.search_filter)"
    } else {
        "*"
    }
    
    Write-Log -Message "Search pattern: $searchPattern" -LogPath $logPath
    
    try {
        $files = Get-ChildItem -Path $srcPath -Filter $searchPattern -File
        Write-Log -Message "Total files found: $($files.Count)" -LogPath $logPath
        
        # Додаткова діагностика
        $allFiles = Get-ChildItem -Path $srcPath -File
        Write-Log -Message "All files in directory: $($allFiles.Count)" -LogPath $logPath
        foreach ($file in $allFiles) {
            Write-Log -Message "File in directory: $($file.Name)" -LogPath $logPath
        }
        
        $files = $files | Where-Object { $_.Name -ne $Config.config_file_name }
        Write-Log -Message "Files after filtering: $($files.Count)" -LogPath $logPath
        foreach ($file in $files) {
            Write-Log -Message "Found file: $($file.FullName)" -LogPath $logPath
        }
    } catch {
        Write-Log -Message "Error occurred while searching for files: $($_.Exception.Message)" -LogPath $logPath -Level "ERROR"
        return
    }
    ...
    foreach ($file in $files) {
        Write-Log -Message "Processing file: $($file.FullName)" -LogPath $logPath
        
        if ($Config.search_filter_type -eq "name-regex" -and $file.Name -notmatch $Config.search_filter) {
            Write-Log -Message "$((Get-LocalizedString -Key "FileIgnored" -Lang $lang)) $($file.Name)" -LogPath $logPath
            continue
        }
        ...
        try {
            if ($file.Name -match '_(\d{6})') {
                ...
                Write-Log -Message "Extracted date from filename: $fileDate" -LogPath $logPath
            } else {
                $fileDate = $file.LastWriteTime
                Write-Log -Message "Using last write time as file date: $fileDate" -LogPath $logPath
            }
            ...
            Write-Log -Message "Destination folder: $destinationFolder" -LogPath $logPath
            
            $destinationPath = Join-Path -Path $destinationFolder -ChildPath $file.Name
            Write-Log -Message "Destination path: $destinationPath" -LogPath $logPath
            
            Move-Item -Path $file.FullName -Destination $destinationPath -Force
            Write-Log -Message "$((Get-LocalizedString -Key "FileProcessed" -Lang $lang)) $($file.Name)" -LogPath $logPath
        } catch {
